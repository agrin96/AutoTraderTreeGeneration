...
def score_decisions(
    ...
    while idx < best_ask.shape[0]:
        ...
        if prev == "SELL" and decisions[idx] == "BUY":
            ...
            virtual_balance = coin_balance*best_bid[idx]*(1-fee)
            prev = "BUY"
            idx += 1
            continue

        if prev == "BUY" and decisions[idx] == "SELL":
            new_balance = coin_balance*best_bid[idx]*(1-fee)
            if new_balance > virtual_balance:
                profitable_trades.append(new_balance - virtual_balance)
            else:
                losing_trades.append(abs(new_balance - virtual_balance))
            valid_trades += 1
...
def calculate_fitness(pop:Dict,
    ...
    # Reward positive profit
    if pop["trades"] != 0 and profit > 0:
        profit = profit*pop["trades"]
    
    pop_depth = tree_depth(pop["tree"])
    depth_modifier = np.power(min(max_depth/pop_depth,1),2)
